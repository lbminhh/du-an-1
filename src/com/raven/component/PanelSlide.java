...
//                  loading.getData().getIdEmployee()
                View.getJframe(loading.getData().getIdEmployee()).setVisible(true);
                //  Close login form
                fram.dispose();
            }
        });
    ...
    private void login(String userName, String password) {
        th = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    Thread.sleep(2000);
                    if (accountServiceImpl.checkLogin(userName, password)) {
                        AccountResponse accountResponse = accountServiceImpl.getAccountByUsername(userName);
                        loading.doneLogin(accountResponse);
                    } else {
                        loading.showError("Tài khoản hoặc mật khẩu không chính xác!");
                    }
